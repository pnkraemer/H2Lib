...
pblock
build_strict_block2(pcluster rc, pcluster cc, void *eta, admissible admis)
{
  pblock    b;
...
//    leafsize = m*m;
    ...
    km = new_kernelmatrix(dim, msize, m, cpos);
    ...
    for(j=0;j<dim;j++){
        ...
        if(cpos==true){
        for(i=points; i<msize; i++){
        	km->x[i][j] = (i - points + 1.0) * 1.0; /* Random points in [-1,1]^2 */
        }}
	}
    printf("\nfine\n");
