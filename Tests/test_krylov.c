...
#include "krylovsolvers.h"  /* for solve_rpgmres_... */
...
main()
{
    ...
    while (steps < 2 * n && residualnorm_pgmres(rhat, k) > 1e-8) {
        if (k + 1 >= kmax){
            finish_rpgmres((addeval_t) addeval_amatrix_avector, A, gauss_seidel, A, b, x, rhat, q, &k, qr, tau);
        }

        step_rpgmres((addeval_t) addeval_amatrix_avector, A, gauss_seidel, A, b, x, rhat, q, &k, qr, tau);
        steps++;
        printf("    Step %u: dimension %u, preconditioned residual %.2e\n", steps, k, ABS(rhat->v[k]));
    }
    if (k>0){
