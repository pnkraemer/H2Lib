...
pkernelmatrix
new_kernelmatrix(uint dim, uint points, uint m, bool cpos)
{
  pkernelmatrix km;
  real *x0;
  uint i;
  ...
  msize = points + (uint)(km->cpos) * (dim + 1);
  km->x = (real **) allocmem(sizeof(real *) * msize);
  km->x[0] = x0 = allocreal(msize * dim);
  for(i=1; i<msize; i++) {
    x0 += dim;
    km->x[i] = x0;
  }
...
creategeometry_kernelmatrix(pckernelmatrix km)
{
  ...
  /* Case 1: kernel cond. pos. def. */
  if(km->cpos == true){
    pbsize = 1 + dim;
    cg = new_clustergeometry(dim, points + pbsize);

    for(i=0; i<points; i++)
      for(j=0; j<dim; j++)
        cg->x[i][j] = cg->smin[i][j] = cg->smax[i][j] = x[i][j];

    for(i=points; i<points + pbsize; i++){
      for(j=0; j<dim; j++){
        cg->x[i][j] =cg->smin[i][j] = cg->smax[i][j] = x[i][j]; 
        cg->smin[i][j] = -1000.0;
        cg->smax[i][j] = 1000.0;
...
//            printf("ii = %i, jj = %i\n", ii, jj);
      ...
      assert(cols <= km->points + (uint) km->cpos * (km->dim + 1));
    ...
    assert(rows <= km->points + (uint) km->cpos * (km->dim + 1));
...
pcluster
build_adaptive_cluster_kernelmatrix(pclustergeometry cf, uint size, uint * idx, uint clf)
{
  pcluster  t;
